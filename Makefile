...
LDFINAL_vmlinux := $(LD)
LD		:= $(LDGOLD)
LDFLAGS		+= -Wl,-dead_strip -plugin LLVMgold.so
...
# CLANG uses a _KBUILD_CFLAGS += $(call cc-disable-warning, unused-but-set-variable)MergedGlobals as optimization, but this breaks modpost, as the
